document.addEventListener('DOMContentLoaded', () => {
    if (!window.File || !window.FileReader || !window.FileList || !window.Blob) {
        alert('Ваш браузер не поддерживает File API. Пожалуйста, обновите браузер.');
        return;
    }
...
function getDocFromDb(fop) {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = (event) => {
            ...
            getRequest.onerror = () => reject("Error retrieving template from IndexedDB");
...
function formatAccountNumber() {
    ...
    const feedbackElement = this.nextElementSibling || document.createElement('span');
    ...
    fetch(`https://openiban.com/validate/${this.value}?getBIC=true&validateBankCode=true`)
...
function validateFormData(formData) {
    ...
    if (!isValidDate(formData.contractDate) || !isValidDate(formData.contractEndDate)) {
...
async function patchDocxInBrowser(templateFile, formData) {
    try {
        ...
        const formattedEndContractDate = formatDateUkrainian(formData.contractEndDate);
        const bankName = formatBankName(formData.bank, formData.bankAbbreviation);

        const fopLastName = formData.fop.split(' ')[0];

        const patches = createPatches(formData, formattedContractDate, formattedEndContractDate, bankName, fopLastName);
        ...
        const blob = new Blob([patchedDoc], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
        ...
        alert("Произошла ошибка при изменении документа. Пожалуйста, попробуйте снова.");
    }
}
...
function createPatches(formData, formattedContractDate, formattedEndContractDate, bankName, fopLastName) {
    return {
